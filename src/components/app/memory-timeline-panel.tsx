...
function metric(item: BelifeMemoryTimelineItem) {
  if (typeof item.confidence === "number") return `confidence ${Math.round(item.confidence * 100)}`;
  if (typeof item.salience === "number") return `salience ${Math.round(item.salience * 100)}`;
    ...
    <section className="rounded-md border border-white/[0.08] bg-white/[0.04] p-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500">BELIFE가 최근에 저장하고 해석한 신호입니다.</p>
        </div>
      </div>
      ...
      <div className="mt-4 space-y-3">
        {timeline.items.length ? (
          timeline.items.map((item) => (
            <article key={`${item.kind}-${item.id}`} className="rounded-md border border-white/[0.08] bg-black/40 p-3">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {item.evidenceType ? (
                      <span className="rounded-sm bg-orange-500/10 px-2 py-1 text-[11px] uppercase text-orange-200">
                        {item.evidenceType}
                      </span>
                    ) : null}
                  </div>
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-zinc-500">
                ...
                {item.tags.slice(0, 4).map((tag) => (
                  <span key={tag} className="rounded-sm border border-white/[0.08] px-2 py-1">
                    {tag}
                  </span>
                ))}
              </div>
          ...
          <div className="rounded-md border border-white/[0.08] bg-black/40 p-4 text-sm leading-6 text-zinc-500">
            아직 표시할 기억 신호가 없습니다.
          </div>
        )}
      </div>

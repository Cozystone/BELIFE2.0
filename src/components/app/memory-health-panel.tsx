  ...
  return (
    <section className="rounded-md border border-white/[0.08] bg-white/[0.04] p-4">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
        ...
        <div className="rounded-md border border-teal-300/15 bg-black/40 px-4 py-3 text-right">
          <p className="text-xs text-zinc-500">Memory health</p>
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
        <div className="space-y-4">
          <ScoreBar label="Freshness" value={report.freshness.score} tone="teal" />
          <ScoreBar label="Evidence clarity" value={1 - report.evidenceBalance.ambiguityRatio} />
          <ScoreBar label="Contradiction inverse" value={1 - Math.min(1, report.contradictionWatchlist.length / 5)} tone="zinc" />
          <div className="grid gap-3 sm:grid-cols-3">
            {report.freshness.windows.map((window) => (
              <div key={window.label} className="rounded-md border border-white/[0.08] bg-black/40 p-3">
                <p className="text-xs text-zinc-500">{window.label} signals</p>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <article className="rounded-md border border-orange-300/10 bg-orange-500/[0.04] p-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-medium text-zinc-100">Contradiction watch</h3>
            </div>
            <div className="mt-3 space-y-3">
              {report.contradictionWatchlist.slice(0, 3).map((item) => (
                <div key={item.id} className="rounded-md border border-white/[0.08] bg-black/30 p-3">
                  <div className="flex items-center justify-between gap-3">
                    <p className="text-sm font-medium text-zinc-100">{item.label}</p>
                    <span className="font-mono text-xs text-orange-200">{item.severity}</span>
                  </div>
                  <p className="mt-2 text-sm leading-6 text-zinc-400">{item.reason}</p>
                </div>
              ))}
              {!report.contradictionWatchlist.length ? (
                <p className="text-sm leading-6 text-zinc-500">No contradiction watch items in recent memory.</p>
              ) : null}
            </div>
          </article>
          ...
          <article className="rounded-md border border-white/[0.08] bg-black/30 p-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-medium text-zinc-100">Forgetting candidates</h3>
            </div>
            <div className="mt-3 space-y-3">
              {report.forgettingCandidates.slice(0, 3).map((item) => (
                <div key={item.id} className="rounded-md border border-white/[0.08] bg-black/35 p-3">
                  <div className="flex items-center justify-between gap-3">
                    <p className="text-sm font-medium text-zinc-100">{item.label}</p>
              ...
              {!report.forgettingCandidates.length ? (
                <p className="text-sm leading-6 text-zinc-500">No stale low-confidence candidates in recent memory.</p>
              ) : null}
            </div>
          </article>
        </div>
      ...
      <div className="mt-5 grid gap-3 lg:grid-cols-[1fr_0.9fr]">
        <article className="rounded-md border border-teal-300/10 bg-teal-400/[0.04] p-4">
          <p className="text-xs font-medium uppercase text-teal-200">Episodic anchors</p>
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            {report.episodicAnchors.slice(0, 4).map((anchor) => (
              <div key={anchor.id} className="rounded-md border border-white/[0.08] bg-black/30 p-3">
                <div className="flex items-center justify-between gap-3">
                  <p className="text-xs font-medium uppercase text-orange-200">{anchor.source}</p>
        ...
        <article className="rounded-md border border-white/[0.08] bg-black/30 p-4">
          <p className="text-xs font-medium uppercase text-zinc-400">Next actions</p>

    ...
    <form onSubmit={submit} className="space-y-4">
      {isSignUp ? (
        <label className="block">
          <span className="text-sm text-zinc-300">이름</span>
          <input
            value={displayName}
      ...
      {!nativeAvailable ? (
        <p className="rounded-md border border-cyan-400/30 bg-cyan-500/10 p-3 text-sm leading-6 text-cyan-100">
          라이브 인증에는 Neon/DATABASE_URL이 필요합니다. 로컬 데모 모드를 계속 사용할 수 있습니다.
        </p>
      ) : null}
      ...
      <div className="flex items-start gap-2 text-xs leading-5 text-zinc-500">
        ...
        비밀번호는 scrypt로 해시하고, 세션은 서버 측 해시 토큰으로 Neon에 저장됩니다.
      </div>
    </form>
  );
}
